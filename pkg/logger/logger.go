...
type ignoreSyncer struct {
	zapcore.WriteSyncer
}

func (i ignoreSyncer) Sync() error {
	err := i.WriteSyncer.Sync()
	if err != nil && os.Getenv("APP_ENV") != "development" {
	}
	return nil
}

func NewIgnoreSyncer(ws zapcore.WriteSyncer) zapcore.WriteSyncer {
	return ignoreSyncer{ws}
}
...
func (l *apiLogger) InitLogger() {
	...
	// file, err := os.OpenFile(fmt.Sprintf("./logs/%d.log", time.Now().Unix()), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	// if err != nil {
	// 	l.sugarLogger.Error(err)
	// }

	logWriter := zap.CombineWriteSyncers(NewIgnoreSyncer(os.Stdout))

	var encoderCfg zapcore.EncoderConfig
	if l.cfg.App.Env == "development" {
